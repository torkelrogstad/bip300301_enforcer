...
fn handle_m3_propose_bundle(
    ...
    sidechain_number: SidechainNumber,
    m6id: [u8; 32],
) -> Result<Option<HandleM3ProposeBundleResult>, HandleM3ProposeBundleError> {
    if dbs
        .sidechain_numbers
        .sidechain
        .try_get(rwtxn, &sidechain_number)?
        .is_none()
    {
        return Ok(Some(HandleM3ProposeBundleResult::InactiveSidechain {
            sidechain_number,
        }));
    }
    ...
    Ok(Some(HandleM3ProposeBundleResult::Valid { m6id }))
}
        ...
        match message {
            ...
            CoinbaseMessage::M3ProposeBundle {
                ...
                match handle_m3_propose_bundle(rwtxn, dbs, sidechain_number, bundle_txid)? {
                    Some(HandleM3ProposeBundleResult::Valid { m6id }) => {
                        tracing::info!(
                    ...
                    Some(HandleM3ProposeBundleResult::InactiveSidechain { sidechain_number }) => {
                        tracing::warn!(
